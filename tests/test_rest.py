...
class test_rest():
    def setup(self):
        self.flaskServer = subprocess.Popen(['python', os.path.join(os.path.dirname(postgis_crud_rest.__file__), 'rest.py'), f'{connectionString}'])
        time.sleep(2)
    ...
    def test_create_update_get_delete(self):
        r = requests.post("http://127.0.0.1:5000/api/create_or_update_polygon", data={'name': 'test', 'id': 1})
        assert r.status_code == 201, 'create_polygon'

        r = requests.post("http://127.0.0.1:5000/api/create_or_update_polygon", data={'name': 'main', 'id': 1})
